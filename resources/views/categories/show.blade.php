...
@section('meta_title', $metaTitle ?? $category->name . ' - AltoParque')
@section('meta_description', 'AltoParque WhatsApp ✅ 11 7178-9529 | ' . ($category->description ?? 'Trabajos de ' . $category->name . ' realizados en zona norte con fotos antes/después.') . ' | ' . $category->name . ', zona norte, CABA')
...
@php
    ...
    if ($categoryFaqs) {
        $serviceSchema = [
            ...
            "name" => $category->name . ' - AltoParque',
            "description" => $category->description ?? $category->name . '. Servicio profesional en Zona Norte de Buenos Aires y CABA. Respuesta rápida, presupuesto sin cargo.',
            "url" => url()->current(),
            "provider" => [
                "@type" => "LocalBusiness",
                "name" => "AltoParque",
